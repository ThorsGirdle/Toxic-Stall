-- Yes, this file is a mess, I'm not touching it again. I appreciate you Emma
local StallTile = assert(SMODS.load_file("src/settings/tile.lua"))()
local map = STALL.LIST_UTILS.map
          ...
          second_row,
          {
            ...
            nodes = {
              create_option_cycle({
                ...
                opt_callback = "stall_update_config_page",
                scale = 0.8,
                colour = G.C.RED,
...
function G.FUNCS.stall_update_config_page(e)
  if not e or not e.cycle_config then return end

  local grid_wrap = G.OVERLAY_MENU:get_UIE_by_ID("stall_grid_wrap")
...
function SMODS.current_mod.config_tab()
  ...
  return {
    ...
    nodes = {
      { n = G.UIT.O, config = { id = "stall_grid_wrap", object = grid } }
    }
  }
end